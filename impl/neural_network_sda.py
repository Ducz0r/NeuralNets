#-------------------------------------------------------------------------------
# Name:        impl.neural_network_sda
# Purpose:     A Theano implementation of a neural network with stacking denoising autoencoders.
#
# Author:      Luka Murn
#
# Created:     2. sep. 2013
...
import time
import cPickle
from impl.sda import StackedDenoisingAutoencoders

# Global variables (for debugging)
MODE = 'FAST_RUN' # 'FAST_COMPILE', 'FAST_RUN', 'DebugMode', 'ProfileMode'
#theano.config.compute_test_value = 'warn'

class NeuralNetworkWithSda(object):
    """
    ...
    :param sda_corruption_level: the corruption level for weight pre-training (can be 0. or None)
    :type sda_corruption_level: float
    
    :param sda_learning_rate: learning rate for training stacked auto-encoders
    :type sda_learning_rate: float
    
    :param sda_batch_size: the mini-batch size for weight pre-training; if None, no minibatches are used
    :tyoe sda_batch_size: int
    
    :param sda_n_epochs: maximal number of epochs to run the weight initializer
    :type sda_n_epochs: int
    ...
    def __init__(self, hidden_levels = [], in_out = (5, 5), learning_rate = 0.01, L1_reg = 0.00, L2_reg = 0.0001,
                 ...
                 sda_corruption_level = 0., sda_learning_rate = 0.01, sda_batch_size = 20, sda_n_epochs = 500, verbose = True):
        ...
        self._sda_corr_lev = sda_corruption_level
        self._sda_learning_rate = sda_learning_rate
        self._sda_batch_size = sda_batch_size
        self._sda_n_epochs = sda_n_epochs
    ...
    def learn(self, train_set, valid_set, test_set = None):
        ...
        sda = StackedDenoisingAutoencoders(hidden_levels = self._hidden_levels, in_dim = self._layer_sizes[0], learning_rate = self._sda_learning_rate,
                                          batch_size = self._sda_batch_size, corruption_level = self._sda_corr_lev, n_epochs = self._sda_n_epochs, seed = self._seed,
                                          verbose = self._verbose)
        
        # Learn the denoising auto-encoders
        if self._verbose: 
            print "============================================================="
            print "= Pre-training weights & biases using stacked auto-encoders ="
            print "============================================================="
        
        sda.learn(train_set[0])
        ws, bs, _ = sda.get_parameters()
