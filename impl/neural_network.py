#-------------------------------------------------------------------------------
# Name:        impl.neural_network
# Purpose:     A Theano implementation of a neural network.
#
# Author:      Luka Murn
#
# Created:     2. sep. 2013
...
class NeuralNetwork(object):
    ...
    :param activation: non linearity to be applied in the hidden layers, can be None
    ...
    def __init__(self, hidden_levels = [], in_out = (5, 5), learning_rate = 0.01, L1_reg = 0.00, L2_reg = 0.0001,
                 dropout_thresholds = (1., 1.), batch_size = 20, n_epochs = 1000, activation = T.nnet.sigmoid, seed = 1234,
                 es_patience = 10000, es_patience_inc = 2, es_improvement_thresh = 0.995, verbose = True):
    ...
    def learn(self, train_set, valid_set, test_set = None):
        ...
        self._weights = []
        self._biases = []
        self._params = [] # Weights and biases represents the parameters of the model
        ...
        #self._cost = T.mean(T.nnet.categorical_crossentropy(self._p_y_given_x, self._y)) + self._L1_reg * self._L1 + self._L2_reg * self._L2_sqr
        ...
        for param, gparam in zip(self._params, self._gparams):
            self._updates.append((param, param - self._learning_rate * gparam))
...
# TWO ADDITIONAL POSSIBLE ACTIVATION FUNCTIONS

def rectified_linear(x):
    """A rectified linear neuron implementation."""
    return T.maximum(0.0, x)

def tanh_modified(x):
    """Modified tanh as suggested by Y. LeCun for efficient back-propagation algorithm."""
